	...
	$.fn.countdown.begin = function (parent, settings) {
		...
		if (!settings.init) {
			...
			$.each(timespan, function (k, v) {
				...
				// Create time element
				time = $('<div/>').addClass('section-background__time').text(v < 10 ? '0' + v : v.toLocaleString());

				if (settings.labels) {
					// Create label element
					label = $('<div/>').addClass('section-background__label').text((v === 1 ? $.fn.countdown.singularize(k) : k));
			...
			// Update each element
			$.each(timespan, function (k, v) {
				$('.section-background__time', '#' + k).text(v < 10 ? '0' + v : v.toLocaleString());
				$('.section-background__label', '#' + k).text((v === 1 ? $.fn.countdown.singularize(k) : k));
			});
		}
